...
class KAN(nn.Module):
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        for _ in pbar:
            ...
            if _ % log == 0:
                pbar.set_description("train loss: %.3f | test loss: %.3f | reg: %.3f " % (torch.sqrt(train_loss).cpu().detach().numpy(), torch.sqrt(test_loss).cpu().detach().numpy(), reg_.cpu().detach().numpy()))
                pbar.set_description("train acc: %.3f | test acc: %.3f | reg: %.3f " % ((train_acc).cpu().detach().numpy(), (test_acc).cpu().detach().numpy(), reg_.cpu().detach().numpy()))
